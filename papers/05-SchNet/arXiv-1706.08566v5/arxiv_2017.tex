...
%  \texttt{kristof.schuett@tu-berlin.de} \\
%  \And
%  Pieter-Jan Kindermans \\
%  Technische Universität Berlin \\
%  \texttt{p.kindermans@tu-berlin.de} \\
%  \And
%  Huziel E. Sauceda\\
%  Fritz-Haber-Institut der Max-Planck-Gesellschaft, Berlin\\
%  \texttt{sauceda@fhi-berlin.mpg.de} \\
%  \And
%  Stefan Chmiela \\
%  TU Berlin,
%  \texttt{@tu-berlin.de} \\
...
%  \texttt{alexandre.tkatchenko@uni.lu} \\
%  \And
%  Klaus-Robert Müller \\
%  Technische Universität Berlin\\
%  \texttt{klaus-robert.mueller@tu-berlin.de}
%}
...
%Machine learning has been used successfully to speed up computationally expensive quantum-chemical calculations
% combined with the vast size of chemical compound space are hampering the discovery of novel molecules with desired properties.
%In recent years, machine learning has been proposed as a tool to speed up exploration of this chemical compound space.
%
%
%However, while convolutional neural networks have transformed the machine learning community they can only be applied to data with a grid structure (e.g. images, audio and video). Atoms, on the other hand, can appear at arbitrary points in space.
%To make convolutional neural networks able to model such data we introduce the continuous filter convolutional layer. 
%We use this layer in the SchNet architecture. This architecture respect fundamental quantum mechanical principles in its predictions. 
%It is also rotationally, translational invariant.  
%We evaluate SchNet on two existing datasets and obtain state of the art performance. 


%Designing high performance deep neural network representations for complex data is a challenge, that can be alleviated if prior domain knowledge is used efficiently. We propose a novel {\em continous filter} convolutive neural network (CF-CNN) architecture, able to avoid inaccuracies that may arrive through its standard discrete filter predecessor. While, clearly, the CF-CNN is a novel ready to use deep network, this model is intuitively motivated by the incorporation of prior knowledge from chemistry. We could demonstrate record performance for two important quantum chemistry applications (a) molecular dynamics and (b) property prediction across chemical compound space, where both compositional (chemical) and configurational (structural) degrees of freedom need to be modeled.
\end{abstract}
...
%\item SchNet achieves state-of-the-art performance on the QM9 benchmark~\citep{ramakrishnan2014quantum}.
%\item Using both energies and forces for the training, we significantly reduce the error for out-of-equilibrium predictions.
%We demonstrate this on a set of eight molecular dynamics trajectories (MD17)~\citep{chmiela2017machine}.
...
%$(e_1(\rr_i-\rr_j),\ldots,e_k(\rr_i-\rr_j))$
...
%Arriving at the final representation after three interactions blocks, we an atom-wise output network in order to predict molecular energies.
%This consists a hidden atom-wise layer with softmax activation reducing the number of features to $32$ and another linear output layer that predicts atom-wise energy contribution.
%The final energy is obtained by sum pooling over all atoms.
%This design ensures that the energy -- as an extensive property -- scales with the number of atoms.
...
%This is an extension to the isomer data set used in \cite{schutt2017quantum} which did not contain forces.
